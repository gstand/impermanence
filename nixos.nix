...
let
  ...
  createDirectories =
    pkgs.runCommand "persistence-create-directories" { buildInputs = [ pkgs.bash ]; }
      ''
  ...
  dirCreationScript =
    let
      ...
      homeDirs = foldl' (
        ...
        in
        if dir.home != null then if !(elem homeDir state) then state ++ [ homeDir ] else state else state
      ) [ ] explicitDirs;

      # Persistent storage directories. These need to be created
      # unless they're at the root of a filesystem.
      persistentStorageDirs = foldl' (
        state: dir:
        let
      ...
      mkParentDirs =
        ...
        let
          ...
          mkParents = dir: map (mkParent dir) (parentsOf dir.directory);
        in
  ...
  persistFileScript = pkgs.writeShellScript "persistence-persist-files" ''
    _status=0
    trap "_status=1" ERR
...
in
{
  ...
  config = mkMerge [
    ...
    (mkIf (allPersistentStoragePaths != { }) (mkMerge [
      {
        systemd.services =
          let
            ...
            services =
              ...
              ++ lib.optional useSystemdActivation {
      ...
      # Assertions and warnings
      {
        assertions =
          let
            markedNeededForBoot =
              cond: fs:
              if config.fileSystems ? ${fs} then config.fileSystems.${fs}.neededForBoot == cond else cond;
        ...
        warnings =
          ...
          in
          mkIf (any id allPersistentStoragePaths.enableWarnings) (mkMerge [
